...
public class MessageExchangeServer {
    ...
    private final ConcurrentMap<ClientAddress, MsgWorker> clients;
...
//        executor.submit(this::acceptSocketConnections);
    ...
    private void clientHandshake(MsgWorker client) {
        ...
        ClientAddress address = new ClientAddress(handshake.getClientType());
    ...
    private ClientAddress getRandomAddresstByType(ClientType type) {
        ...
        return (ClientAddress) values[generator.nextInt(values.length)];
    }
    ...
    private void exchange() {
        while (true) {
            for (Map.Entry<ClientAddress, MsgWorker> client : clients.entrySet()) {
                ...
                while (msg != null) {
                    ...
                    ClientAddress to = (msg.getTo() instanceof ClientAddress)
                        ? (ClientAddress) msg.getTo()
                        // TODO: handle NPE!
                        : getRandomAddresstByType(msg.getTo().getType());
